...
time required to build the index file should be minimized. Therefore, 
our main criteria for benchmark are set as below:
...
and ``youtube.txt.vids`` is file containing the dataset metadata information. 
...
The above data shows QPS(Queries Per Second) values for each accuracy values. Both N2 and nmslib shows similar search performance.
...
N2 builds index file faster and uses less memory than nmslib, while having a similar search performance.

The benchmark environment uses multiple threads for index builds but a single thread for searching.
In a real production environment, you will need to run concurrent searches by multiple processes or multiple threads.
